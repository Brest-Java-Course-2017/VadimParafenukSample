...
public class UsersConsumerRestMockTest {
    ...
    public void getUserById() throws Exception {
        expect(mockRestTemplate.getForEntity(hostUrl + "/" + urlUser + "/id" + 3, User.class))
                .andReturn(new ResponseEntity<User>(user3, HttpStatus.FOUND));
        replay(mockRestTemplate);

        usersConsumer.getUserById(3);
        assertEquals(USER_LOGIN_3, user3.getLogin());
    }

    @Test
    public void addUser() throws Exception {
        expect(mockRestTemplate.postForEntity(hostUrl + "/" + urlUser , user3, Integer.class))
                .andReturn(new ResponseEntity<Integer>(3, HttpStatus.CREATED));
        replay(mockRestTemplate);

        Integer userId = usersConsumer.addUser(user3);
        assertEquals(userId, (Integer)3);
    }
